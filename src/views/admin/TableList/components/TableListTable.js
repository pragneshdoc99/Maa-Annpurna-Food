...
export default function TableListTable(props) {
  ...
  //     const databaseFld = firebase.firestore();
  //     let docRef = databaseFld.collection("table-list")
  //     docRef = docRef.where("tableName", "==", deletionVal);
  //     docRef.get().then((querySnapshot) => {
  //       querySnapshot.forEach((doc) => {
  //         // console.log("USER DOC ID ::  ", doc.id);
  //         databaseFld.collection("table-list").doc(doc.id).delete().then(() => {
  ...
  const filterByValue = (array, value) =>
    array.filter(
      (data) =>
        ...
        data.tableType.toLowerCase().includes(value.toLowerCase())
    );
  ...
  return (
    <CardCHk
                ...
                <div className="overflowtable">
                  ...
                  {!loader && noDataImg && (
                    <Table className={classes.table}>
                      <TableHead>
                        <TableRow>
                          <TableCell className={"txtleft"}>{"Table"}</TableCell>
                          <TableCell className={"txtleft"}>{"Type"}</TableCell>
                          <TableCell></TableCell>
                          {/* <TableCell></TableCell> */}
                        </TableRow>
                      </TableHead>
                      <TableBody>
                        {myArray.map((product) => (
                          ...
                          >
                            <TableCell
                              align={"left"}
                              className={product.tableStatus ? classes.active : classes.inactive}
                            >
                              {product.tableName}
                            </TableCell>
                            <TableCell
                            ...
                            >
                              ...
                              >
                                <EditIcon
                                  onClick={() => editThisFeed(product.tableName, product)}
                                  variant="contained"
                                  size="sm"
                                  style={{ color: "#20382b" }}
                                />
                                ...
                                <DeleteIcon
                                  ...
                                  onClick={() =>
                                    setConfirmDialog({
                                      ...
                                      onConfirm: () => {
                                        handleDelete(product.tableName);
                                      },
                                    })
                                  }
                                ></DeleteIcon>
      ...
      {detailForm && !currentUserId ? <AddTable /> : ""}

      {currentUserId && detailForm ? (
        <AddTable currentUserId={currentUserId} dataDetails={listData} />
      ) : (
        ""
      )}
